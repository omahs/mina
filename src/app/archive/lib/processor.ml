...
module For_test = struct
  ...
  let breadcrumb_as_runtime_config breadcrumb = 
    let create_ledger_as_list ledger =
          let%map accounts = Mina_ledger.Ledger.to_list ledger in
          List.map accounts ~f:(fun acc ->
              Genesis_ledger_helper.Accounts.Single.of_account acc None )
        in
    let staged_ledger =  Transition_frontier.Breadcrumb.staged_ledger breadcrumb
          in
          let%bind accounts = create_ledger_as_list (Staged_ledger.ledger staged_ledger) in
  ...
  let breadcumb_as_precomputed_block_with_attributes breadcrumb ~precomputed_values ~logger = 
    ...
    let precomputed = Transition_frontier.Breadcrumb.For_tests.to_precomputed_block breadcrumb ~precomputed_values ~logger in
              
    let blockchain_length = Transition_frontier.Breadcrumb.For_tests.blockchain_length breadcrumb in
    let blockchain_length = Unsigned.UInt32.to_int blockchain_length in
              
    (precomputed, blockchain_length, state_hash)

  let gen_blocks ~trials ~size ~precomputed_values ~logger ~verifier ~accounts_with_secret_keys ~f=
  ...
  let archive_random_blocks ~trials ~size ~precomputed_values ~logger ~verifier ~accounts_with_secret_keys ~pool ~f_check =
    gen_blocks ~trials ~size ~precomputed_values ~logger ~verifier ~accounts_with_secret_keys 
    ~f:(fun breadcrumbs ->
  ...
  let dump_blocks ~size ~precomputed_values ~logger ~verifier ~accounts_with_secret_keys ~path = 
    gen_blocks  ~size ~precomputed_values ~logger ~verifier ~accounts_with_secret_keys 
    ~f:(fun breadcrumbs -> 
    ...
    let save_runtime_config runtime_config = 
      ...
      Yojson.Safe.to_file (String.concat [path; "/"; "runtime_config.json"]) json 
    in
    ...
    List.iter breadcrumbs ~f:(fun breadcrumb -> 
      let precomputed = Transition_frontier.Breadcrumb.For_tests.to_precomputed_block breadcrumb ~logger ~precomputed_values in
      let state_hash = Transition_frontier.Breadcrumb.state_hash breadcrumb in
      let state_hash = Mina_base.State_hash.to_base58_check state_hash in 
      let blockchain_length = Transition_frontier.Breadcrumb.For_tests.blockchain_length breadcrumb in 
      let blockchain_length = Unsigned.UInt32.to_int blockchain_length in 
      let filename = Printf.sprintf "mainnet_%d_%s.json" blockchain_length state_hash in
